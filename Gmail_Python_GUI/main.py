...
def send_email():
    if toEntryField.get()=='' or subjectEntryField.get()=='' or textarea.get(1.0,END)=='\n':
        messagebox.showerror('Thông báo','Bạn phải nhập đủ tất cả trường này!',parent=root)

    else:
        if choice.get()=='single':
            result=sendingEmail(toEntryField.get(),subjectEntryField.get(),textarea.get(1.0,END))
            if result=='sent':
                messagebox.showinfo('Thông báo','Email đã gửi thành công')

            if result=='failed':
                messagebox.showerror('Thông báo lỗi','Email không thể gửi.')
        ...
        if choice.get()=='multiple':
            ...
            messagebox.showinfo('Thông báo','Emails đã gửi thành công')
...
def settings():
    ...
    def save():
        if fromEntryField.get()=='' or passwordEntryField.get()=='':
            messagebox.showerror('Thông báo','Bạn phải nhập đủ tất cả trường này!',parent=root1)

        else:
            f=open('credentials.txt','w')
            f.write(fromEntryField.get()+','+passwordEntryField.get())
            f.close()
            messagebox.showinfo('Thông báo','ĐÃ LƯU ĐỊA CHỈ CỦA BẠN',parent=root1)
    ...
    Label(root1,text='Đăng nhập Gmail của bạn',image=logoImage,compound=LEFT,font=('times new roman',32,'bold'),
          fg='white',bg='gray20').grid(padx=40)
...
def iexit():
    result=messagebox.askyesno('Thông báo','Bạn muốn thoát chương trình không?')
    if result:
        root.destroy()
    else:
        pass
...
titleLabel=Label(titleFrame,text='  Email Sender of NGUYEN NGOC THUONG',image=logoImage,compound=LEFT,font=('Goudy Old Style',20,'bold'),
...
emailLabelFrame=LabelFrame(root,text='Nội dung Email',font=('times new roman',16,'bold'),bd=5,fg='white',bg='dodger blue2')
emailLabelFrame.grid(row=4,column=0,padx=20)
micImage=PhotoImage(file='mic.png')

Button(emailLabelFrame,text='',compound=LEFT,font=('arial',12,'bold'),
...
Button(emailLabelFrame,text=' Tệp đính kèm',image=attachImage,compound=LEFT,font=('arial',12,'bold'),
